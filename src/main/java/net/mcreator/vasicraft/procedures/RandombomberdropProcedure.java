...
public class RandombomberdropProcedure {
			...
			if (local_drop > 0.3 && local_drop <= 0.6) {
				...
				if (local_drop > 0.6 && local_drop <= 0.9) {
					...
					if (local_drop > 0.93 && local_drop <= 0.96) {
						if (world instanceof ServerLevel _level) {
							ItemEntity entityToSpawn = new ItemEntity(_level, x, y, z, new ItemStack(VasiCraftModItems.EXPLOSIVEGRENADE.get()));
							entityToSpawn.setPickUpDelay(10);
							_level.addFreshEntity(entityToSpawn);
						}
					} else if (local_drop > 0.96 && local_drop <= 0.97) {
						if (world instanceof ServerLevel _level) {
							ItemEntity entityToSpawn = new ItemEntity(_level, x, y, z, new ItemStack(VasiCraftModItems.VASI_PHONE.get()));
							entityToSpawn.setPickUpDelay(10);
							_level.addFreshEntity(entityToSpawn);
						}
					}
