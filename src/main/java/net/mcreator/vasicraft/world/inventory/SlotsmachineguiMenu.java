...
import net.mcreator.vasicraft.procedures.Credits_labelProcedure;
...
public class SlotsmachineguiMenu extends AbstractContainerMenu implements Supplier<Map<Integer, Slot>> {
	...
	public SlotsmachineguiMenu(int id, Inventory inv, FriendlyByteBuf extraData) {
		super(VasiCraftModMenus.SLOTSMACHINEGUI.get(), id);
		...
		this.customSlots.put(17, this.addSlot(new SlotItemHandler(internal, 17, 178, 7) {
			...
			@Override
			public boolean mayPlace(ItemStack stack) {
				return VasiCraftModItems.WALLET.get() == stack.getItem();
			}
		}));
		for (int si = 0; si < 3; ++si)
			for (int sj = 0; sj < 9; ++sj)
				this.addSlot(new Slot(inv, sj + (si + 1) * 9, 14 + 8 + sj * 18, 35 + 84 + si * 18));
		for (int si = 0; si < 9; ++si)
			this.addSlot(new Slot(inv, si, 14 + 8 + si * 18, 35 + 142));
	}
	...
	public static void onPlayerTick(TickEvent.PlayerTickEvent event) {
		...
		if (event.phase == TickEvent.Phase.END && entity.containerMenu instanceof SlotsmachineguiMenu) {
			Level world = entity.level();
			double x = entity.getX();
			double y = entity.getY();
			double z = entity.getZ();
			Credits_labelProcedure.execute(world, entity);
		}
	}
}
